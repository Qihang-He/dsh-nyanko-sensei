...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * state. `ms` is the clip's true duration — it comes from the authored
		 * frame count and frame rate, and is what the state machine times
		 * one-shots against.
		 *
		 * These are produced by deforming a single illustration (tools/
		 * gen_motion.py), so they are grouped by what the puppet can express:
		 * body-wide squash and stretch, a whole-body lean, a head tilt, and an
		 * ear flick. There is deliberately no leg articulation, because one
		 * drawing cannot supply it honestly.
		 */
		const ANIMS = {
			// Ambient loops the pet falls back into.
			idle: { loop: true, ms: 800, pool: "ambient" },
			breathe_deep: { loop: true, ms: 500, pool: "ambient" },
			look_around: { loop: true, ms: 625, pool: "ambient" },
			walk: { loop: true, ms: 667, pool: "ambient" },
			sleep: { loop: true, ms: 1333, pool: "ambient" },
			// One-shot reactions, each returning to a resting pose.
			ear_flick: { loop: false, ms: 667, next: "idle" },
			hop: { loop: false, ms: 667, next: "idle", voice: ["happy"] },
			bounce_land: { loop: false, ms: 667, next: "idle" },
			happy: { loop: false, ms: 500, next: "idle", voice: ["natsume", "happy"] },
			angry: { loop: false, ms: 500, next: "idle", voice: ["angry"] },
			surprised: { loop: false, ms: 667, next: "idle", voice: ["surprised"] },
			spin: { loop: false, ms: 667, next: "idle" },
			// Held while the pointer owns the pet.
			drag: { loop: true, ms: 800 },
		};

		/**
		 * Where an animation the *story* wants is mapped onto one the puppet can
		 * actually perform.
		 *
		 * The motion set is built from one drawing, so some beats a pet would
		 * normally have — a yawn, eating, a full body transformation — have no
		 * honest source clip. Mapping them onto the nearest real motion keeps the
		 * behaviour legible instead of silently dropping the beat, and keeps the
		 * declared list in step with the assets on disk so the package check has
		 * no false alarms.
		 */
		const ANIM_ALIASES = {
			blink: "ear_flick",
		...
		class NyankoSensei {
			...
			play(name, options = {}) {
				if (this.destroyed) return;
				// Resolve through the alias table first, then verify the asset is
				// actually on disk: a rendered set can be a subset of the declared
				// one, and playing a missing file just leaves a blank box.
				const wanted = ANIM_ALIASES[name] ?? name;
				if (this.hasAnim(wanted)) name = wanted;
				else if (this.hasAnim(name)) { /* the alias itself was unnecessary */ }
				else name = "idle";
			 ...
			 *
			 * The pool is weighted by repetition rather than by a weight field:
			 * `idle` appearing three times makes it the most likely resting state,
			 * which is what a real pet looks like, while the occasional look-around
			 * or nap keeps it alive.
			 */
			enterIdle(preferred) {
				if (this.destroyed) return;
				const pool = [
				...
				if (name === "sleep") this.say("sleepy", { chance: 0.4 });
			}
			...
			/**
			 * What a left click does: pick a reaction, play it, and speak.
			 *
			 * The roll favours the cheerful reactions, because a pet that mostly
			 * gets annoyed at being petted is unpleasant to keep on screen. The
			 * region decides which line comes with it, so the same reaction reads
			 * differently depending on where it was touched.
			 */
			clickReaction(region) {
				const roll = Math.random();
				let name = "happy";
				if (roll > 0.92) name = "surprised";
				else if (roll > 0.82) name = "angry";
				else if (roll > 0.66) name = "hop";
				else if (roll > 0.5) name = "spin";
				this.react(name);
				// `happy` and `hop` carry their own voice list; anything else speaks
				// its own clip when one exists, and falls back to the signature line.
				if (!(ANIMS[name] && ANIMS[name].voice)) {
					const has = this.voiceUrl(name);
					this.speak(has ? name : this.settings.clickVoice);
				}
				if (region === "head") this.say("happy", { chance: 0.5 });
				else if (region === "tail") this.say("angry", { chance: 0.5 });
			}
			...
			bindGlobal() {
				...
				this.on(stage, "pointerdown", (event) => {
					...
					// Pointer capture keeps a fast drag tracking after the cursor leaves
					// the sprite, but it throws when there is no live pointer for the id
					// — synthetic events from tests, and some touch sequences. Losing
					// capture only costs tracking outside the box, so it must not abort
					// the drag itself.
					try {
